...
function mergeClasses(defaultClasses, customClasses) {
  ...
  return [
    ...new Set(Object.values(defaultClassMap)),
    ...customClassList.filter((cls) => !defaultClassMap[cls.split("-")[0]]),
  ].join(" ");
}
...
export function Typography({ variant, children, className = "" }) {
  const baseClasses = {
    ...
    h2: `scroll-m-20 border-b  text-3xl font-semibold tracking-tight first:mt-0`,
  ...
  return (
    <ComponentTag style={{ margin: "0" }} className={mergedClassName}>
      {children}
    </ComponentTag>
  );
}
