...
export default function ProjectFilter() {
  ...
  return (
    <section className="w-[17rem]">
      <Card
        className={`p-5 pb-0 sticky top-10 rounded-none ${
          theme == "dark" ? "bg-gray-900" : "bg-secondary"
        }`}
      >
        ...
        <CardContent className="px-0 py-0 no-scrollbar">
          <ScrollArea className="mt-4 h-[71.7vh] no-scrollbar overflow-auto">
            {filters.map((filter, index) => (
              <div key={index} className="mb-4">
                <Typography
                  variant="h4"
                  className="pb-3 dark:text-gray-400 border-b"
                >
                  {filter.name}
                </Typography>
                <div className="pt-5">
                  ...
                  >
                    {filter.options.map((option, idx) => (
                      <div
                        key={idx}
                        className="flex items-center space-x-2 first:-mt-1 mt-2"
                      >
                        <RadioGroupItem
                          value={option.value}
                          id={`${filter.name}-${idx}`}
